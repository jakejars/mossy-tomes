...
type GeneratorMode = 'Quest Seed' | 'Adventure Situation';
type HookType = 'Patron Hook' | 'Supernatural Hook' | 'Happenstance Hook';
type LevelTier = '1-4' | '5-10' | '11-16' | '17-20';

// --- QUEST SEED INTERFACES ---
interface GeneratedQuestSeed {
  hook: string;
  objective: string;
  target: string;
  location: string;
...
// --- ADVENTURE SITUATION INTERFACES ---
interface GeneratedAdventureSituation {
  levelTier: LevelTier;
  situation: string;
}

// --- MAIN DATA INTERFACE ---
interface QuestGenData {
  hookTypes: HookType[];
...
export default function QuestGeneratorPage() {
  ...
  // Generator Mode
  const [generatorMode, setGeneratorMode] = useState<GeneratorMode>('Quest Seed');
  ...
  const copyToClipboard = () => {
    ...
    if (generatorMode === 'Quest Seed' && generatedQuestSeed && isCompleteQuestSeed(generatedQuestSeed)) {
      if (generatedQuestSeed.hook) text += `Hook: ${generatedQuestSeed.hook}\n\n`;
      if (generatedQuestSeed.objective) text += `Objective: ${generatedQuestSeed.objective}\n`;
      if (generatedQuestSeed.target) text += `Target: ${generatedQuestSeed.target}\n`;
      if (generatedQuestSeed.location) text += `Location: ${generatedQuestSeed.location}\n\n`;
      if (generatedQuestSeed.complication) text += `Complication: ${generatedQuestSeed.complication}\n`;
      if (generatedQuestSeed.climax) text += `Climax: ${generatedQuestSeed.climax}\n`;
  ...
  const ComponentWithControls = ({ 
    ...
    <div className="relative group mb-4 pr-20">
      ...
      <div className="absolute right-0 top-1/2 -translate-y-1/2 flex gap-2 opacity-20 group-hover:opacity-100 transition-opacity">
        <button
          onClick={() => toggleQuestLock(componentKey)}
          className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${
            lockedQuestSeed[componentKey]
              ? 'bg-moss-600 text-white border-moss-700'
              : 'bg-moss-800/30 text-moss-400 border-moss-700/30'
          } border hover:bg-moss-600 hover:text-white`}
        ...
        <button
          ...
          className="w-8 h-8 rounded-full bg-moss-800/30 text-moss-400 border border-moss-700/30 flex items-center justify-center hover:bg-moss-600 hover:text-white transition-all"
          title="Reroll"
        >
          ↻
        </button>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
                  ...
                  <button
                    ...
                    className={`flex-1 py-2 px-4 rounded-lg transition-all ${
                      generatorMode === 'Quest Seed'
                        ? 'bg-moss-600 text-white border-moss-700'
                        : 'bg-moss-800/30 text-moss-300 border-moss-700/30 hover:bg-moss-700/30'
                    } border`}
                  >
                    Quest Seed
                  </button>
              ...
              {generatorMode === 'Quest Seed' && (
                <>
                  <div className="mb-4">
                    ...
                    >
                      <option value="Patron Hook">Patron Hook</option>
                      <option value="Supernatural Hook">Supernatural Hook</option>
                      <option value="Happenstance Hook">Happenstance Hook</option>
                    </select>
                  </div>
                ...
                <>
                  <div className="mb-6">
                    ...
                    <select
                      ...
                      className="w-full p-2 bg-moss-900/50 border border-moss-700/30 rounded-lg text-moss-100"
                    >
                      <option value="1-4">Levels 1-4 (Apprentice)</option>
                      <option value="5-10">Levels 5-10 (Journeyman)</option>
                      <option value="11-16">Levels 11-16 (Master)</option>
                      <option value="17-20">Levels 17-20 (Epic)</option>
                    </select>
                  </div>
                  <button
            ...
            {/* Quest Seed Display */}
            {generatorMode === 'Quest Seed' && generatedQuestSeed && isCompleteQuestSeed(generatedQuestSeed) && (
              <div className="card p-8">
                ...
                <div className="space-y-4">
                  ...
                  {(generatedQuestSeed.complication || generatedQuestSeed.climax) && (
                    <hr className="border-moss-700/30 my-4" />
                  )}
                  {generatedQuestSeed.complication && (
                    <ComponentWithControls
                      label="Complication"
          ...
          <div className="space-y-6">
              ...
              {showEditor ? (
                ...
                <div className="space-y-4 text-moss-300">
                  ...
                  <ul className="space-y-2 text-sm">
                    <li>• <strong className="text-moss-200">Quest Seed Mode:</strong> Hook types & hooks, objectives, targets, locations, complications, climaxes/twists</li>
                    <li>• <strong className="text-moss-200">Adventure Situation Mode:</strong> Level tiers, situations by level (1-4, 5-10, etc.)</li>
                  </ul>
                </div>
              )}
            </div>
            ...
            <div className="card p-6">
              ...
              <ul className="space-y-2 text-moss-300 text-sm">
                <li>• Use <strong className="text-moss-200">Quest Seed</strong> mode for creating specific, structured quests with clear goals</li>
                <li>• Use <strong className="text-moss-200">Adventure Situation</strong> for broader, level-appropriate inspiration for a new adventure arc</li>
                <li>• In Quest Seed mode, use <strong className="text-moss-200">Lock (🔒)</strong> to keep a good hook or complication while rerolling other parts</li>
                <li>• A <strong className="text-moss-200">Climax</strong> is often more useful than a simple twist as it suggests a final scene</li>
                <li>• Combine a generated <strong className="text-moss-200">Calamity</strong> from the Settlement generator with a quest to fix it</li>
